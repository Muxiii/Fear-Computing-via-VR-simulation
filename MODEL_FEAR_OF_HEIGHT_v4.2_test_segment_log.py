...
SEG_STARTS = [0, -0.5, -1, -2, -3]
SEG_ENDS = [15, 10, 8, 6, 4]
...
def raw_data_segmentation(biopac_file,time_list,seg_start =SEG_START, seg_end=SEG_END):
    eda_segmented=[]
    ecg_segmented=[]
    ppg_segmented=[]
    output_timelist=[]
...
if __name__ == "__main__":
    ...
    for segment_start in SEG_STARTS:
        for segment_end in SEG_ENDS:

            if segment_end - segment_start < 8:
                continue

            print(f"Segment:{segment_start}s to {segment_end}s, Loading data...")
            t1 = time.time()
                ...
                for file_ in os.listdir(participant_files_path):
                    ...
                    if (file_.find("BIOPAC_data") != -1):
                        ...
                        for time_ in time_list:
                            timestamps_sub, raw_eda_sub, raw_ecg_sub, raw_ppg_sub = raw_data_segmentation(
                                raw_data_file_path,
                                time_, segment_start, segment_end)
                ...
                for i in range(1, 4):
                    ...
                    for item in os.listdir(sub_file):
                        if (item == "1.csv"):
                            ...
                            pupil_l_seg, pupil_r_seg, eye_l_seg, eye_r_seg = load_eye_data(filename,
                ...
                for i in range(1, 4):
                    ...
                    for item in os.listdir(sub_file):
                        if ((".csv" in item) & (item != "1.csv")):
                            ...
                            pupil_r_seg_decouple = [pupil_r_seg[i] - decouple_coefficient * pupil_r_lightreflex[i] for i
                                                    in
                                                    range(0, len(pupil_r_lightreflex))]
                            pupil_l_seg_decouple = [pupil_l_seg[i] - decouple_coefficient * pupil_l_lightreflex[i] for i
                                                    in
                                                    range(0, len(pupil_l_lightreflex))]
            ...
            for participant in participants:
                ...
                for i in range(len(participant.timestamps)):
                    participant.eda_processed.append(
                        eda_filter_normalize(participant.raw_eda[i], participant.timestamps[i]))
                    ...
                    time.sleep(0.1)
                    tonic, phasic = cvxEDA(participant.eda_processed[i], delta=sample_delta, tau0=2., tau1=0.7,
                                           delta_knot=10.,
            ...
            for participant in participants:
                ...
                ecg_features_df.columns = ['heartrate_mean', 'heartrate_std', "hrv_MeanNN", "hrv_SDNN", "hrv_RMSSD",
                                           "hrv_SDSD",
                                           "hrv_MedianNN", "hrv_Prc20NN", "hrv_Prc80NN", "hrv_pNN50", "hrv_pNN20",
                                           "hrv_MinNN",
                                           "hrv_MaxNN"]
            ...
            for participant in participants:
                ...
                df = pd.DataFrame(
                    {"max_peak_height": max_peaks, "avr_peak_amp": avr_amp, "phasic_mean": mean_phasic,
                     "phasic_std": std_phasic,
            ...
            for participant in participants:
                ...
                for i in range(0, len(participant.pupil_left)):
                    ...
                    pupil_change.append(
                        np.mean(pupil[0: int(0.4 * len_seg)]) - np.mean(pupil[int(0.6 * len_seg): int(1 * len_seg)]))
                ...
                df_eye = pd.DataFrame(
                    {"pupil 0-0.2": pupil_0, "pupil 0.2-0.4": pupil_1, "pupil 0.4-0.6": pupil_2,
                     "pupil 0.6-0.8": pupil_3,
                     "pupil 0.8-1": pupil_4,
                     "pupil_change 0-1": pupil_change, "pupil_std": pupil_std, "eye_mean": eye_mean,
                     "eye_std": eye_std})
            ...
            # RandomForestRegressor (indiv. seperately)
            mses = []
            for participant in participants:
                x_train, x_test, y_train, y_test = train_test_split(participant.features, participant.labels,
                                                                    train_size=24)
            ...
            mses = np.array(mses)
            file = open('seg_test_log.txt', 'a')
            print(f"Segment:{segment_start}s to {segment_end}s, indiv Random Forest RMSE mean {(mses ** 0.5).mean()}\n")
            file.write(f"Segment:{segment_start}s to {segment_end}s, indiv Random Forest RMSE mean {(mses ** 0.5).mean()}\n")
            ...
            print(f"Segment:{segment_start}s to {segment_end}s, Random Forest bi-accuracy: {statistics.mean(acc)}\n")
            #print("RandomForestRegressor2 5-fold cross validation:")
            print(f"Segment:{segment_start}s to {segment_end}s, Random Forest RMSE mean: {- statistics.mean(rmse)}\n")
            file.write(f"Segment:{segment_start}s to {segment_end}s, Random Forest bi-accuracy: {statistics.mean(acc)}\n")
            file.write(f"Segment:{segment_start}s to {segment_end}s, Random Forest RMSE mean: {- statistics.mean(rmse)}\n")
            ...
            print(f"Segment:{segment_start}s to {segment_end}s, Linear Regression bi-accuracy: {statistics.mean(acc2)}\n")
            print(f"Segment:{segment_start}s to {segment_end}s, Linear Regression RMSE mean: {- statistics.mean(rmse2)}\n\n")
            file.write(f"Segment:{segment_start}s to {segment_end}s, Linear Regression bi-accuracy: {statistics.mean(acc2)}\n")
            file.write(f"Segment:{segment_start}s to {segment_end}s, Linear Regression RMSE mean: {- statistics.mean(rmse2)}\n\n")
            file.close()
