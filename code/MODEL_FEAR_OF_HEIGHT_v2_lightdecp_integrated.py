...
def raw_data_segmentation(biopac_file,time_list):
    ...
    for i in range(1,len(time_list)):
        df_list.append(df[(df["timestamp"]>=timestamps[i]) & (df["timestamp"]<(timestamps[i]+30))])
    for i in range(10):
...
x_train,x_test,y_train,y_test=train_test_split(features_total,labels_total, train_size=216)
...
print(f"Random Forest binary classification accuracy: {accuracy}")
...
predicts = reg2.predict(x_test)
# print(pd.DataFrame([y_test.to_list(),predicts]))
print(f"RMSE: {mean_squared_error(predicts,y_test)**0.5}")
mean_predictor=np.full(len(predicts),4)
labels_total.median()
fear_labels=np.array(y_test>4)
fear_predictions=np.array(predicts>4)
accuracy2 =(fear_labels==fear_predictions).mean()
print("Linear Regression binary classification accuracy:", accuracy2)
rmse_scores = cross_val_score(
reg2, features_total,labels_total, cv=5,scoring='neg_root_mean_squared_error' )
...
regs=GridSearchCV(estimator=RandomForestRegressor(), scoring='neg_root_mean_squared_error',cv=5,
             param_grid={'max_depth':[5,10,20,50,500],'min_samples_leaf':[1,2,3,5,8,10],
                         'max_features':["sqrt","auto","log2"],'min_impurity_decrease':[0,0.1,0.3,1]})
...
result=pd.DataFrame(regs.cv_results_)                    #不同参数下的结果 research rabbit
regs.best_params_
...
labels_total.head(200)


# In[ ]:


for i in range(len(participants[0].eda_phasic)):
    plt.figure()
    plt.plot(participants[0].timestamps[i], participants[0].eda_phasic[i])
